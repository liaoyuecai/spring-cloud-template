...
public final class TccManager {
    ...
    public static TccManager transaction = new TccManager();

    public static Map<String, Object> serviceMap = new HashMap();
    public static Map<String, Class[]> paramMap = new HashMap();
    ...
    public static synchronized void init(String serverName, String packageName) {
        ...
        try {
            Set<Class<?>> set = getClasses(packageName);
            Service s;
            Class[] classes;
            String serviceName;
            for (Class<?> clazz : set) {
                s = clazz.getAnnotation(Service.class);
                if (s != null) {
                    serviceName = s.value();
                    if (StringUtils.isNotEmpty(serviceName)) {
                        serviceMap.put(serviceName, SpringUtil.getBean(serviceName));
                        Method[] methods = clazz.getMethods();
                        for (Method method : methods) {
                            classes = method.getParameterTypes();
                            if (classes != null && classes.length > 0)
                                paramMap.put(serviceName + "." + method.getName(), classes);
                        }
                    }
     ...
     * @return
     */
    static Set<Class<?>> getClasses(String pack) throws ClassNotFoundException, IOException {
        ...
        while (dirs.hasMoreElements()) {
            ...
            if ("file".equals(protocol)) {
                ...
                // 如果是jar包文件
                // 定义一个JarFile
                JarFile jar = ((JarURLConnection) url.openConnection()).getJarFile();       //获取到jar文件
                // 从此jar包 得到一个枚举类
     ...
     * @param classes
     */
    static void findAndAddClassesInPackageByFile(String packageName, String packagePath, final boolean recursive, Set<Class<?>> classes) throws ClassNotFoundException {
        // 获取此包的目录 建立一个File
